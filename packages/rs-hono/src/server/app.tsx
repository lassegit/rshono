...
import type { Route, StaticRoute, DynamicRoute, EndpointRoute } from "../router.js";
import { renderToStream } from "../renderer/ssr.js";
...
export function buildApp(options: BuildAppOptions): Hono {
  ...
  const pageRoutes = routes.filter(
    (r): r is StaticRoute | DynamicRoute =>
      r.kind === "static" || r.kind === "dynamic"
  );
  const endpointRoutes = routes.filter(
    (r): r is EndpointRoute => r.kind === "endpoint"
  );
  ...
  internalApp.route(
    "/_static",
    createStaticMiddleware({ roots: staticRoots, isDev })
  );
...
function createPageApp(
  ...
  for (const route of routes) {
    app.get(route.path, async (c) => {
      ...
      if (route.loader) {
        try {
          loaderData = await route.loader(c);
        } catch (err) {
      ...
      try {
        ...
        console.error(
          `[rs-hono] Loader data for ${route.path} is not JSON-serializable:`,
          err
        );
