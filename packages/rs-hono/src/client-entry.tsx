...
import type { Route, StaticRoute, DynamicRoute } from "./router.js";
...
async function bootstrap() {
  ...
  if (!data) return; // not an rs-hono page (e.g. error page)
  ...
  const route = (routes as Route[]).find(
    (r): r is StaticRoute | DynamicRoute =>
      (r.kind === "static" || r.kind === "dynamic") && r.path === data.route
  );
  if (!route) {
