#!/usr/bin/env node
/**
 * rsc-hono CLI bootstrap.
 *
 * Runs the TypeScript CLI directly via tsx — no pre-build step. tsx is
 * resolved from rsc-hono's own dependencies (never from PATH), so this
...
try {
  ...
  console.error("rsc-hono: could not resolve its 'tsx' dependency. Try reinstalling rsc-hono.");
  process.exit(1);
}
...
child.on("error", (err) => {
  console.error("rsc-hono: failed to start:", err.message);
  process.exit(1);
});
