...
const HELP = `rsc-hono — Hono + Rspack + React Server Components

Usage:
  rsc-hono dev     [--port 3000]   start the dev server
  rsc-hono build                   build for production (client + server + SSG)
  rsc-hono start   [--port 3000]   run the production build
...
async function main(): Promise<void> {
    ...
    if (values.version) {
        ...
        console.log(require('rsc-hono/package.json').version);
        return;
    }
    ...
    if (values.port && Number.isNaN(port)) {
        console.error(`rsc-hono: invalid --port "${values.port}"`);
        process.exit(1);
    }
    ...
    switch (command) {
        ...
        default:
            console.error(`rsc-hono: unknown command "${command}"\n`);
            console.log(HELP);
            process.exit(1);
    }
}
