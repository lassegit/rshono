 ...
 *      /_rsc-hono/hmr  SSE: 'client-built' after client rebuilds,
...
export async function devCommand(options: DevOptions): Promise<void> {
    ...
    serverCompiler.hooks.invalid.tap('rsc-hono/gate', () => {
        workerGate = createGate();
    });

    serverCompiler.hooks.done.tapPromise('rsc-hono/worker', async (stats: Stats) => {
    ...
    clientCompiler.hooks.done.tap('rsc-hono/hmr', (stats: Stats) => {
        if (stats.hasErrors()) {
            console.error(stats.toString({ preset: 'errors-warnings', colors: true }));
            return;
        }
    ...
    serve({ fetch: front.fetch, port, hostname: '127.0.0.1' }, (info) => {
        console.log(`  ➜ rsc-hono dev server: http://localhost:${info.port}`);
    });
}
