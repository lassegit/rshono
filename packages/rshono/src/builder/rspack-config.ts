 ...
 * ('@rshono/routes'), so its `import()` thunks are the code-split
 ...
 * and @hono/node-server are bundled so the runtime needs no rshono
 * install; rshono itself ships TypeScript source.
 */
...
export function createConfigs(options: ConfigOptions): [RspackOptions, RspackOptions] {
    ...
    const resolveBase = {
        ...
        // USER modules; that package is a dependency of rshono, not of
    ...
    const serverConfig: RspackOptions = {
        ...
        externals: [
            ({ request }, callback) => {
                ...
                // '@rshono/' aliases, the allowlisted packages, and the
