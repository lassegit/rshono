...
> is therefore weighted toward *primitives every non-trivial app needs* (auth/session access,
...
| …but SSR-side leak-proofing | ✅ | ⚠️ | …the SSR shadow only covers files that *literally open* with `'use client'`. A secret read in a plain helper leaks into SSR HTML. **Verified.** See BUGS.md #1. |
...
- **Env/secret model for the client bundle.** Build-time replacement is a *harder* guarantee than
...
1. **No request context (`cookies()`/`headers()`) in server components or actions.** This is the
   single biggest blocker: you cannot render per-user content or implement session auth *inside* the
...
- **Middleware wraps pages.** `app.route('/', serverApp)` is registered before the page routes, and
  ...
  So global middleware in `src/server.ts` *does* observe and can short-circuit page rendering — auth
...
Measured against its own thesis — *stability and DX, not feature count* — rshono is already a
